...
public class PickerNetThreeDialog
{
    ...
    public PickerNetThreeDialog(Context context, PickerCallback callback)
    {
        this(context, null, callback);
    }

    public PickerNetThreeDialog(Context context, final String title, PickerCallback callback)
    {
    ...
    private void init(List<PickerBean> list)
    {
        pickerView = new OptionsPickerNetView(new OptionsPickerNetView.Builder(context)
                .setLayoutRes(R.layout.picker_layout, new CustomListener()
                {
                    ...
                    public void customLayout(View v)
                    {
                        ...
                        textTail3 = v.findViewById(R.id.textTail3);
                        setTail(tail1, tail2, tail3);
                    }
                })
                ...
                .setTextColorOut(0xffbbbbbb))
        {
            ...
            public void onSelect(int options1, int options2, int options3, PickerBean item1, PickerBean item2, PickerBean item3)
            {
                try
                {
                    if(pickerCallback.onSelect(options1, options2, options3, item1, item2, item3))
                    {
                        index1 = options1;
                        index2 = options2;
                        index3 = options3;
            ...
            {
                pickerCallback.onLoadList3(pi1, pi2, parent, onLoadOkListener);
            }
        };
        ArrayList<PickerBean> list2 = new ArrayList<>();
        list2.add(new PickerBean(""));
        ArrayList<PickerBean> list3 = new ArrayList<>();
        list3.add(new PickerBean(""));
        pickerView.setPicker(list, list2, list3);
    }
    ...
    private void loadData(final boolean isShow)
    {
        if(!isLoad)
        {
            ...
            pickerCallback.onLoadList1(new OnLoadOkListener()
            {
                @Override
    ...
    public void setTail(String tail1, String tail2, String tail3)
    {
        this.tail1 = tail1;
        this.tail2 = tail2;
        this.tail3 = tail3;
        if(textTail3 != null)
        {
            ...
            textTail3.setText(tail3);
            textTail1.setVisibility(tail1 == null? View.GONE: View.VISIBLE);
            textTail2.setVisibility(tail2 == null? View.GONE: View.VISIBLE);
            textTail3.setVisibility(tail3 == null? View.GONE: View.VISIBLE);
        }
    }

    public void select(int index1, int index2, int index3)
    {
        pickerView.setSelectOptions(index1, index2, index3);
    }
    ...
    {
        boolean onSelect(int index1, int index2, int index3, PickerBean bean1, PickerBean bean2, PickerBean bean3);

        void onLoadList1(OnLoadOkListener onLoadOkListener);

        void onLoadList2(int pi1, PickerBean parent1, OnLoadOkListener onLoadOkListener);

        void onLoadList3(int pi1, int pi2, PickerBean parent, OnLoadOkListener onLoadOkListener);
    }
}